...
function DisplayListing() {
  ...
  const getLatestListing = async () => {
    try {
      ...
      // Fetch the latest listings with associated images
      const { data: listingsData, error: listingsError } = await supabase
        ...
        .select(`
          ...
          listingimages (url)
        `)
        .order("created_at", { ascending: false }) // Sort by latest created listings
        .limit(10); // Get the latest 10 listings

      if (listingsError) throw listingsError;

      setListings(listingsData); // Store all listings
    } catch (err) {
  ...
  style={{
    backgroundImage: "url('/pexels-marketingtuig-87223.jpg')", // Correct Next.js public folder path
  ...
  <div className="px-10 md:px-36 flex-grow  bg-opacity-60">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {filteredListings.map((listing) => (
        <div
          key={listing.id}
          className="p-6 rounded-lg shadow-md bg-transperent border border-gray-700"
        >
          ...
          {/* Listing Details */}
          <h3 className="font-bold text-xl mb-4 text-zinc-300">
            ₹{listing.price}
          </h3>
